...
void finish_chunk_map() {
    ...
    fprintf(stderr, "[DEBUG] finish_chunk_map called with chunk_count = %d\n", chunk_count);
}
...
//std::vector<ChunkResult> all_chunk_results;
    ...
    for (int seg_id = 0; seg_id < NUM_STREAMS + BUFFER_COUNT - 1; ++seg_id) {
        ...
        if (process_seg_id >= 0) {
            ...
            //fprintf(stderr, "[DEBUG] segment_offset: %zu\n", current_segment_offset);
            ...
            #pragma omp parallel num_threads(MAX_THREADS)
            {
                ...
                for (int i = 0; i < CHUNKS_PER_STREAM; i++) {
                    if (h_chunk_results[process_buffer][i].valid && 
                        ...
                        ChunkResult *res = &h_chunk_results[process_buffer][i];
                        const char *chunk_id = res->chunk_id;
                        size_t chunk_size = res->size;
                        size_t global_offset = res->offset;
                        size_t local_offset = global_offset - current_segment_offset;
                        uint8_t *chunk_buf = h_pinned_buffers[process_buffer] + local_offset;
                        if (chunk_buf < h_pinned_buffers[process_buffer] ||
                            chunk_buf + chunk_size > h_pinned_buffers[process_buffer] + SEGMENT_SIZE) {
                            //fprintf(stderr, "[DEBUG] segment_offset: %zu, global_offset: %zu, local_offset: %zu, chunk_size: %zu\n", current_segment_offset, global_offset, local_offset, chunk_size);
                            continue;
                        }

                        
                        int target = i % MAX_THREADS;
                        ...
                        if (send_chunk_over_connection(sockfd, chunk_id, chunk_buf, chunk_size) == 0) {
                            ...
                            //fprintf(stderr, "[+] 전송 성공: chunk %s (%zu bytes) -> %s:%d\n",
                            //        chunk_id, chunk_size, node_ips[target], node_ports[target]);
                        } else {
                            #pragma omp critical
                            fprintf(stderr, "[-] 전송 실패: chunk %s (%zu bytes) -> %s:%d\n",
                                    chunk_id, chunk_size, node_ips[target], node_ports[target]);
                        }
            ...
            cpu_processing_done[process_buffer] = true;
            //fprintf(stderr, "CUDA kernel error: %s\n", cudaGetErrorString(err));
        }
        // 버퍼 전환
